...
const findValidBattle = (battles: any[], clashTagUser2: string): any | null => {
    return battles.find((battle: any) => {
        const opponentTag = battle.opponent[0]?.tag;
        return opponentTag === `#${clashTagUser2}`;
    });
};
...
export const validMatch = async (user1: string, user2: string,
    ...
    try {
        ...
        if (responseUser1.data.length > 0) {
            ...
            const battle = findValidBattle(responseUser1.data, clashTagUser2);

            if (battle) {

                if (battle.gameMode?.id === 72000007) {
                    ...
                    channel.send(`Partida não é um clássico 1x1`)
                }
            } else{
                channel.send(`Partida não encontrada`)
            }
        } else {
